...
#               prms_l[ind] = shlex.quote(prm)
    ...
    def dlg_config_list(self):
        ...
        while True:
            ...
            DLG_W, DLG_H    = WD_LST+GAP*4, ACTS_T[3]+GAP
            ext_hnzs    = ['{}={}'.format(nm, sz) for (nm,sz) in ext_nz_d.items()]
            ext_vlss    = []
            for ext in self.exts:
                ext_vlss+=[[                                    ext['nm']
                          ...
                          ,                                     ext['lxrs']
                          ,                                     ext['rslt']
                         #,self.rslt_v2c[                       ext['rslt']]
                          ,                                     ext['savs']
                         #,self.savs_v2c[                       ext['savs']]
                          ]]
...
#               if app.msg_box( 'Delete Tool\n    {}'.format(exkys[new_ext_ind])
    ...
    def dlg_config_prop(self, ext, keys=None):
        ...
        while True:
            ...
            # DLG ACTS
            +[C1.join(['type=button'    ,POS_FMT(l=GAP,    t=PROP_T[12],r=GAP+PRP1_W,b=0)
                      ,'cap=Help'
                      ])] # i=25
